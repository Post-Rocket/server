...
const fixAttributes = Fix.fixAttributes = (attributes, model, addPrimaryKeys) => {
  ...
  addPrimaryKeys && attributes.push(...model.primaryKeyAttributes);
	...
	for (let i = 0, l = attributes.length; i !== l; ++i) {
		...
		Array.isArray(attr) && attr.length && (attr[0] = Sequelize.literal(`(${attr[0]})`))
	}
...
const fixWhere = Fix.fixWhere = where => {
	...
	for (let k in (where || {})) {
		...
		if (op && op !== k) {
			where[op] = v;
			delete where[k];
		} else if ((kk = k.toLowerCase()).includes("sql") || kk.includes("raw") || kk.includes("query")) {
			where[k] = Sequelize.literal(d);
...
const fixOrderArr = ["asc", "desc"],
  ...
  fixOrder = Fix.fixOrder = order => (
	Array.isArray(order) && order.map(v => (
		Array.isArray(v) && Sequelize.literal(
...
const fixInclude = Fix.fixInclude = (include, model, sequelize, addPrimaryKeys) => {
	...
	for (let i = 0, l = include.length ; i !== l; ++i) {
		...
		const a = v.association === "string"
		  && (v.association = associations[v.association || ""])
		...
		// Recusrsive call.
		v.include = fixInclude(v.include, a && a.target || model, sequelize, addPrimaryKeys);
	}
...
const fixAssociationPropertyKeys = Fix.fixAssociationPropertyKeys = (obj, model, sequelize) => {
	if (model) {
		const tableAttributes = model.tableAttributes, associations = model.associations;
		for (const k in (tableAttributes && obj || {})) {
			const name = tableAttributes[k] === undefined && k;
			const association = name && associations[name];
			if (!association) continue;
	
			const p = obj[name] = obj[k];
			delete obj[k];
			if (typeof p === "object") obj[name] = fixAssociationPropertyKeys(p, association, sequelize);
		}
	}
